...
[11.24每日一题原题链接](https://leetcode.cn/problems/binary-prefix-divisible-by-5?envType=daily-question&envId=2025-11-26)
[11.25每日一题原题链接](https://leetcode.cn/problems/smallest-integer-divisible-by-k?envType=daily-question&envId=2025-11-26)

以上两道每日一题都有一个相同的特点，成指数级增长，暴力解法必然超时或溢出，我们利用余数相同的数具有相同的整除性质来解决这道题目！

#### 1. 前缀数值的递推关系
对于二进制数组 nums，第 i 个前缀的二进制值 $num_i$ 与第 i-1 个前缀的关系为：$num_i = num_{i-1} \times 2 + nums[i]$

#### 2. 转化为余数的递推
我们需要判断 $num_i \mod 5 == 0$，根据同余定理：
$num_i \mod 5 = (num_{i-1} \times 2 + nums[i]) \mod 5 $
$ = [(num_{i-1} \mod 5) \times (2 \mod 5) + (nums[i] \mod 5)] \mod 5$
简化得：$num_i \mod 5 = (num_{i-1} \mod 5 \times 2 + nums[i]) \mod 5$

#### 3. 用余数代替前缀和
又因为 $ num_{i-1} \mod 5 = prefix_{i-1}$，因而有余数的递推式子  $prefix_i \mod 5 = (prefix_{i-1} \mod 5 \times 2 + nums[i]) \mod 5$
...
[原题链接](https://leetcode.cn/problems/maximum-subarray-sum-with-length-divisible-by-k?envType=daily-question&envId=2025-11-27)
...
#### 快速幂
简单地说就是如果计算$x^n$，`pow(x,n)`计算的时$x*x*...x$耗时较长，我们可以将$n$分解为二进制数，我们只需要计算$x^1, x^2, x^4, x^8, \dots$，然后根据二进制数的每一位是否为1，来判断是否需要乘到结果中。
{%note info%}
