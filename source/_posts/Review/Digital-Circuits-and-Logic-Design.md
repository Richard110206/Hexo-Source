...
{%note info%}
实例：74HC148，74LS148 电路原件中圆圈表示低电平有效
{%endnote%}
...
### 竞争与险象
#### 检查竞争—冒险
竞争的定义是信号经不同路径到达同一点的时间差，只要输出端的逻辑函数在一定条件下能简化成$ Y = A + A'$  或  $Y = A \cdot A' $，则可出现竞争—冒险现象。（卡诺图中，若两个相邻最小项分属不同包围圈）
...
- **阈值特性（555构成时）**
  - 上限阈值：$V_{T+} = \frac{2}{3}V_{CC}$（输入上升至该值时，输出从高→低）；
  - 下限阈值：$V_{T-} = \frac{1}{3}V_{CC}$（输入下降至该值时，输出从低→高）；
  - 滞回电压：$\Delta V_T = V_{T+}-V_{T-} = \frac{1}{3}V_{CC}$。


- **工作原理**
输入信号变化时，通过与上下阈值比较实现状态翻转：
1. 输入$v_I < V_{T-}$：输出$v_O$为高电平（稳态1）；
2. $V_{T-} < v_I < V_{T+}$：输出保持原状态；
3. $v_I > V_{T+}$：输出$v_O$为低电平（稳态2）；
4. $v_I$回落至$V_{T-}$以下时，输出恢复高电平。


- **关键结论**
滞回特性可抑制输入信号中的噪声干扰，仅对幅度超过滞回电压的信号响应。


- **用处**
  - 波形整形：将不规则波形（如正弦波、三角波）转换为矩形波；
  - 噪声抑制：过滤输入信号中的尖峰干扰；
  - 电平转换：将不同幅度的信号转换为标准逻辑电平；
  - 幅度鉴别：仅保留幅度超过上限阈值的输入信号。
...
```
1. 语言字符不区分大小写，但建议使用大写来写关键字
2. 同一类型多个输入用逗号`,`分隔，完整的语句用分号`;`结束，用`-`来注释一行用`% %`来注释一整段
3. 所有的程序并发的执行，不依赖描述的前后顺序
4. 数值不能赋值给节点，必须使用VCC和GND
...
{%note danger%}
注意这里的`fp`是一个D触发器，是时序逻辑电路，不需要每次都指定状态，而下面这段代码是组合逻辑电路（未定义为D触发器），所以在条件判断之后每次都需要指定电平状态！
```AHDL
SUBDESIGN BCDCNT1
(
    inclk    : INPUT;      -- 输入时钟
...
```
{%note info%}
1. 在逻辑段中只能有一个Defaults语句，并且它必须是关键字BEGIN后的第一个语句
2. 如果在一个Defaults语句中为一个变量多次赋值，那么只有最后一次赋值有效
{%endnote%}
...
{%fold into@用模块化设计思想设计一个 60 进制的计数器%}
设计要求：
（1）INK 为 1KHZ 信号输入端；
（2）60 进制计数器只能对秒信号进行计数；
（3）计数器的输出给显示译码器，显示器为 2 位，不需要设计显示译码器。
```AHDL
-- 60进制计数器设计（模块化设计）
-- 首先设计1KHz分频为1Hz的分频器模块
SUBDESIGN freq_divider
(
    CLK        : INPUT;   -- 1KHz输入时钟
    RESET      : INPUT;   -- 复位信号
    SEC_PULSE  : OUTPUT;  -- 1Hz秒脉冲输出
)
VARIABLE
    counter[9..0] : DFF;  -- 10位计数器（计数值0-999）
BEGIN
    DEFAULTS
        -- 所有DFF默认时钟为CLK，清零为RESET
        counter[].clk = CLK;
        counter[].clrn = !RESET;
    END DEFAULTS;
...
-- 个位计数器模块（0-9）
SUBDESIGN ones_counter
(
    CLK        : INPUT;   -- 秒脉冲输入
    RESET      : INPUT;   -- 复位信号
    ONES_OUT   : OUTPUT;  -- 个位输出（BCD码）
    CARRY_OUT  : OUTPUT;  -- 进位输出
)
VARIABLE
    count[3..0] : DFF;  -- 4位BCD计数器
BEGIN
    ...
    ONES_OUT = count[];  -- 输出个位BCD码
END;

-- 十位计数器模块（0-5）
SUBDESIGN tens_counter
(
    CLK        : INPUT;   -- 进位脉冲输入
    RESET      : INPUT;   -- 复位信号
    TENS_OUT   : OUTPUT;  -- 十位输出（BCD码）
    RESET_ALL  : OUTPUT;  -- 60进制复位信号
)
VARIABLE
    count[2..0] : DFF;  -- 3位计数器（0-5只需要3位）
BEGIN
    DEFAULTS
        count[].clk = CLK;
        count[].clrn = !RESET;
    ...
    TENS_OUT = count[];  -- 输出十位BCD码
END;

-- 主模块：60进制计数器
SUBDESIGN counter_60
(
    INK        : INPUT;   -- 1KHz时钟输入
    RESET      : INPUT;   -- 复位信号
    TENS       : OUTPUT;  -- 十位输出（BCD码）
    ONES       : OUTPUT;  -- 个位输出（BCD码）
)
VARIABLE
    -- 内部信号
    sec_clk      : NODE;   -- 1Hz秒脉冲
    ones_carry   : NODE;   -- 个位进位信号
    reset_internal : NODE;  -- 内部复位信号
BEGIN
    -- 实例化分频器：1KHz -> 1Hz
    freq_div1: freq_divider
    PORT MAP (
        CLK => INK,
        RESET => RESET,
        SEC_PULSE => sec_clk
    ...
    -- 实例化十位计数器
    tens_cnt: tens_counter
    PORT MAP (
        CLK => ones_carry,
        RESET => reset_internal,
        TENS_OUT => TENS,
        RESET_ALL => reset_internal
    );
END;

-- 可选：顶层测试模块
SUBDESIGN top_counter
(
    INK        : INPUT;   -- 1KHz时钟输入
    RESET      : INPUT;   -- 复位信号
    SEG1[6..0] : OUTPUT;  -- 十位数码管显示（假设为共阳极）
    SEG2[6..0] : OUTPUT;  -- 个位数码管显示
)
VARIABLE
    tens_bcd[3..0] : NODE;  -- 十位BCD码
    ones_bcd[3..0] : NODE;  -- 个位BCD码
BEGIN
    -- 实例化60进制计数器
    cnt60: counter_60
    PORT MAP (
