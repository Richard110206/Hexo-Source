...
git commit -m "feat: "
```
**解释：**`git commit -m` 命令用于提交更改到本地仓库，并且 `-m` 参数后面跟着的是你为此次提交**添加的描述信息**，也就是提交信息（`commit message`）。提交信息是对此次提交所做更改的**简短描述**，它帮助团队成员或其他查看项目历史的人理解每次提交的目的或内容。
